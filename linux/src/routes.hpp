...
/// Routes view — lists virtual model route rules with add/delete actions.
///
/// GtkStack pages:
///   - "empty" : Adw.StatusPage placeholder
///   - "list"  : Adw.PreferencesGroup with one Adw.ActionRow per route
class RoutesView {
public:
    RoutesView() {
    ...
    /// Refresh the route list from the service.
    void refresh(FireBoxDbusClient& client) {
        ...
        try {
            auto rules = client.get_route_rules();
...
private:
    ...
    static void on_delete_confirmed(AdwMessageDialog* dlg,
        ...
        if (g_strcmp0(response, "delete") == 0 && ctx) {
            try {
                ctx->client->delete_route(ctx->route_id);
    ...
    static void on_add_response(AdwMessageDialog* dlg,
        ...
        if (g_strcmp0(response, "add") == 0 && w) {
            ...
            try {
                w->client->set_route_rules(vm_id, dn, strategy, pid, mid);
                w->view->refresh(*w->client);
            } catch (const std::exception& e) {
                show_error(GTK_WINDOW(dlg), e.what());
            }
        }
