...
use anyhow::Context;
use bytes::Bytes;
use oauth2::{PkceCodeChallenge, RefreshToken};
        ...
        Some(mut creds) => {
            ...
            match refresh_access_token(http, provider_tag, &mut creds, &creds_path).await {
                ...
                Err(e) => {
                    ...
                    device_code_auth(http, provider_tag, &creds_path, event_tx).await?
                }
            }
        }
        None => {
...
/// Refresh the access_token using the refresh_token (via `oauth2` typed exchange).
...
async fn refresh_access_token(
    ...
    // Build the refresh request body using oauth2 types for the token value.
    let refresh_token = RefreshToken::new(creds.refresh_token.clone());

    let form_body = format!(
        "grant_type=refresh_token&refresh_token={}&client_id={}",
        urlencoded(refresh_token.secret()),
...
/// 1. Request device code with PKCE (challenge generated by the `oauth2` crate)
/// 2. Print URL for user authorization
/// 3. Poll for token (DashScope requires `code_verifier` — non-standard extension)
...
async fn device_code_auth(
    ...
    // Generate PKCE pair using the oauth2 crate (replaces hand-rolled sha2/base64).
    ...
    let form_body = format!(
        ...
        urlencoded(pkce_challenge.as_str()),
    );
    ...
    // Emit IPC event so the native layer can show a notification.
    if let Some(tx) = event_tx {
        let _ = tx.send(crate::ipc::IpcEvent::OAuthOpenUrl {
    ...
    // Use the oauth2 PkceCodeVerifier for the code_verifier value.
    let code_verifier_str = pkce_verifier.secret().to_string();
    ...
    loop {
        ...
        match error_code {
            ...
            "slow_down" => {
                ...
                if interval > Duration::from_secs(DEVICE_CODE_MAX_POLL_INTERVAL_SECS) {
                    interval = Duration::from_secs(DEVICE_CODE_MAX_POLL_INTERVAL_SECS);
                }
                debug!(
...
pub async fn preflight_check(
    ...
    ensure_access_token(http, provider_tag, oauth_creds_path, event_tx).await?;
    Ok(())
}
...
mod tests {
    ...
    fn load_dashscope_config() -> Option<crate::config::ProviderConfig> {
        // Tests should use keyring-loaded config, but for backward compat
        // we just return None if no DashScope config exists.
        let config = crate::config::Config::load_from_keyring();
        config
            .providers
            .into_iter()
    ...
    async fn test_token_refresh() {
        ...
        let resolved = ensure_access_token(&http, &ds.tag, creds_path, None)
            .await
