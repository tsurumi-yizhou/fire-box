...
fn create_with_default_url() {
    let provider = AnthropicProvider::new("ant-test-key", DEFAULT_URL);
...
fn create_with_custom_url() {
    let provider = AnthropicProvider::new("ant-custom-key", "http://localhost:9090/v1");
...
fn api_key_is_stored() {
    ...
    let provider = AnthropicProvider::new(key, DEFAULT_URL);
...
fn custom_url_override() {
    ...
    let provider = AnthropicProvider::new("key", custom_url);
...
fn provider_with_empty_api_key() {
    let provider = AnthropicProvider::new("", DEFAULT_URL);
...
fn custom_url_with_different_ports() {
    ...
    for port in ports {
        ...
        let provider = AnthropicProvider::new("key", &url);
...
fn different_instances_different_keys() {
    let provider1 = AnthropicProvider::new("key1", DEFAULT_URL);
    let provider2 = AnthropicProvider::new("key2", DEFAULT_URL);
...
fn different_instances_different_urls() {
    let provider1 = AnthropicProvider::new("key", DEFAULT_URL);
    let provider2 = AnthropicProvider::new("key", "http://custom:8080/v1");
...
fn url_http_allowed_for_local() {
    let provider = AnthropicProvider::new("key", "http://localhost:8080/v1");
...
async fn complete_with_invalid_key_should_fail() {
    let provider = AnthropicProvider::new("invalid-key", DEFAULT_URL);
    let request = CompletionRequest {
        model: "claude-3-sonnet-20240229".to_string(),
        messages: vec![ChatMessage {
            role: "user".to_string(),
